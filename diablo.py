...
This call does not check the board is present or working, under most circumstances use Init() instead
        ...
        self.i2cRead = open("/dev/i2c-" + str(self.busNumber), "rb", buffering = 0)
        ioctl(self.i2cRead, I2C_SLAVE, self.i2cAddress)
        self.i2cWrite = open("/dev/i2c-" + str(self.busNumber), "wb", buffering = 0)
        ioctl(self.i2cWrite, I2C_SLAVE, self.i2cAddress)
...
Displays the names and descriptions of the various functions and settings provided
        """
        funcList = [Diablo.__dict__.get(a) for a in dir(Diablo) if isinstance(Diablo.__dict__.get(a), FunctionType)]
